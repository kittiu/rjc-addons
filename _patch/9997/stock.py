        ...
        for pick in self.browse(cr, uid, ids, context=context):
            ...
            for move in too_few:
                ...
                if not new_picking:
                    ...
                    # Start kittiu - specific for RJC
                    picking_out_obj = self.pool.get('stock.picking.out')
                    picking_out = picking_out_obj.browse(cr, uid, pick.id)
                    is_printed = picking_out and picking_out.printed or False
                    # End kittiu                      
                    ...
                    # Start kittiu - specific for RJC
                    picking_out_obj.write(cr, uid, [pick.id], {'printed': False})
                    # End kittiu                      
                    ...
                    # Start kittiu - specific for RJC
                    picking_out_obj.write(cr, uid, [new_picking], {'printed': is_printed})
                    # End kittiu  
                if product_qty != 0:
                    defaults = {
